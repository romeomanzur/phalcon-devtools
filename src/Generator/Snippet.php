...
EOD;
    ...
    public function getClassDoc(
        ...
        $isPhalconModel = ltrim($extends, '\\') === 'Phalcon\\Mvc\\Model';

        if (!$includeDescription && !$isPhalconModel) {
            return '';
        }
        ...
        if ($includeDescription) {
            ...
            if (!empty($namespace)) {
                $package = str_replace('namespace ', '', $namespace);
                $package = str_replace(';', '', $package);
                $package = str_replace(["\r", "\n"], '', $package);
...
EOD;
    ...
    public function getAttributes($type, $visibility, \Phalcon\Db\ColumnInterface $field, $annotate = false, $customFieldName = null)
    {
        ...
        $isNullable = $field->isAutoIncrement() || !$field->isNotNull();

        $docType = $isNullable && !str_contains((string) $type, 'null')
            ? $type . '|null'
            : $type;
...
EOD;
        ...
        return PHP_EOL . sprintf($templateAttributes, $docType, $visibility, $fieldName) . PHP_EOL;
    }
